...
int toolFun::conf_check(zoey_threadpool_conf_t *conf){
	if (conf == NULL){
		return -1;
	}
...
void toolFun::task_queue_init(zoey_task_queue_t* task_queue){
	
	task_queue->head = NULL;
	task_queue->tail = &task_queue->head;
}

int toolFun::thread_mutex_create(pthread_mutex_t *mutex){
	int ret = 0;
...
void toolFun::thread_mutex_destroy(pthread_mutex_t *mutex){
	
	pthread_mutex_destroy(mutex);
}

int toolFun::thread_cond_create(pthread_cond_t *cond){
	
	return pthread_cond_init(cond, NULL);
}

void toolFun::thread_cond_destroy(pthread_cond_t *cond){
	
	pthread_cond_destroy(cond);
}

int toolFun::threadpool_create(zoey_threadpool_t *pool,toolFun* obj){
	...
	{
		pthread_create(&pid, &attr, obj->threadpool_cycle,pool);
	}	
	pthread_attr_destroy(&attr);
	return 0;
}

void toolFun::threadpool_cycle(void* argv){
...
void toolFun::threadpool_exit_cb(void* argv){
...
int toolFun::thread_add(zoey_threadpool_t *pool,toolFun* obj){
	...
	ret = pthread_create(&pid, &attr, obj->threadpool_cycle,pool);
	if (ret == 0)
	{
		pool->threadnum++;
	}
...
void toolFun::change_maxtask_num(zoey_threadpool_t *pool, unsigned int num){
...
int toolFun::thread_key_create(){
	
	return pthread_key_create(&key, NULL);
}

void toolFun::thread_key_destroy(){
